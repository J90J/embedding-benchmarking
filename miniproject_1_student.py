...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # Input Embedding
        # Note: We use the text_search from session_state
        input_sentence = st.session_state.text_search
        input_vec = averaged_glove_embeddings_gdrive(input_sentence, word_index_dict, embeddings, model_type)
        
        for i, cat in enumerate(categories):
            cat_vec = averaged_glove_embeddings_gdrive(cat, word_index_dict, embeddings, model_type)
        ...
        for i, cat in enumerate(categories):
            # Optimisation: Check cache handled in get_category_embeddings utility? 
            # The template provided get_category_embeddings but didn't explicitly enforce using it here.
            # We will call get_openai_embeddings directly for simplicity as per the Colab logic
