...
Part C: Real-World Applications
-Pick 3 groups of everyday categories (e.g., Kitchen, Furniture, Baby, Outdoor). For this task we use the categories: cinema, hotel, restaurant.
-For each group, write 2 sentences using similar words but different word order. Make it up yoursefl for the categories we created (cinema, hotel, restaurant. Just be creative)

Example:
-cinema hotel restaurant → Category: cinema
-hotel restaurant cinema → Category: hotel
-Observe how word position changes meaning and category assignment.
-Briefly explain why the category changes for each pair.

DO NOT DO ANY OF THIS FOR NOW (NO BONUS stuff for now) 
